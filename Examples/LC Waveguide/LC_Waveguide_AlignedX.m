...
lambda = 1550e-9;
beta0 = 2*pi*1.60/lambda; % Propagation constant will be close to that of free space.
Nx = 1000;
NoModes = 6;
...
function [x, nx, ny, nz] = GenerateFibreProfile(Nx)
    ...
    ne = 1.7072;
    w = 3e-6;
    theta=pi/4; % angle of LC with x axis
    ...
    n(y_mesh < w/2) = ng;
    nx = n; ny = n; nz = n;
    nx(abs(x_mesh) <= w/2 & abs(y_mesh) <= w/2) = sqrt(n0^2*cos(theta)^2 + ne^2*sin(theta)^2);
    ny(abs(x_mesh) <= w/2 & abs(y_mesh) <= w/2) = sqrt(n0^2*sin(theta)^2 + ne^2*cos(theta)^2);
    nz(abs(x_mesh) <= w/2 & abs(y_mesh) <= w/2) = n0;
