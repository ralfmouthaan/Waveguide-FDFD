...
function [RetVal] = ModeSolverFD(dx, n, lambda, beta, NoModes)
    ...
    for i = 1:Nx*Nx
        ...
        if West_Dist <= PML_Depth
            ...
            Sx_Hz = 1-PML_SigmaMax*(1-(West_Dist-0.5)/PML_Depth)^PML_PolyDegree*1i/w/eps0/sqrt(Epsr(i));
        elseif East_Dist <= PML_Depth
            ...
            Sx_Hz = 1-PML_SigmaMax*(1-East_Dist/PML_Depth)^PML_PolyDegree*1i/w/eps0/sqrt(Epsr(i));
        end
        if North_Dist <= PML_Depth
            ...
            Sy_Hz = 1-PML_SigmaMax*(1-(North_Dist-0.5)/PML_Depth)^PML_PolyDegree*1i/w/eps0/sqrt(Epsr(i));
        elseif South_Dist <= PML_Depth
            ...
            Sy_Hz = 1-PML_SigmaMax*(1-South_Dist/PML_Depth)^PML_PolyDegree*1i/w/eps0/sqrt(Epsr(i));
        end
    ...
    %Qxx = Epsy*Ay*invEpsz*Dy + k0^2*Epsy - k0^(-2)*By*Ax*invEpsz*Dy*Cx + k0^(-2)*Bx*Ay*invEpsz*Dy*Cx + Bx*Cx;
    %Qxy = -Epsy*Ay*invEpsz*Dx - k0^(-2)*Bx*Ay*invEpsz*Dx*Cx + k0^(-2)*By*Ax*invEpsz*Dx*Cx + By*Cx;
    %Qyx = -Epsx*Ax*invEpsz*Dy - k0^(-2)*By*Ax*invEpsz*Dy*Cy + k0^(-2)*Bx*Ay*invEpsz*Dy*Cy + Bx*Cy;
    %Qyy = Epsx*Ax*invEpsz*Dx + k0^2*Epsx - k0^(-2)*Bx*Ay*invEpsz*Dx*Cy + k0^(-2)*By*Ax*invEpsz*Dx*Cy + By*Cy;
    
    %Qxx = Cy*By + k0^2*Epsx - k0^(-2)*Ax*invEpsz*Dy*Cx*By + k0^(-2)*Ax*invEpsz*Dx*Cy*By + Ax*invEpsz*Dx*Epsx;
    %Qxy = -Cy*Bx - k0^(-2)*Ax*invEpsz*Dx*Cy*Bx + k0^(-2)*Ax*invEpsz*Dy*Cx*Bx + Ax*invEpsz*Dy*Epsy;
    %Qyx = -Cx*By - k0^(-2)*Ay*invEpsz*Dy*Cx*By + k0^(-2)*Ay*invEpsz*Dx*Cy*By + Ay*invEpsz*Dx*Epsx;
    Qyy = Cx*Bx + k0^2*Epsy - k0^(-2)*Ay*invEpsz*Dx*Cy*Bx + k0^(-2)*Ay*invEpsz*Dy*Cx*Bx + Ay*invEpsz*Dy*Epsy;
    ...
    [eigvectors, eigvalues] = eigs(Qyy, NoModes, beta^2);
    ...
    for i = 1:NoModes
        Ey(:,i) = eigvectors(1:Nx*Nx, i);
        Ex(:,i) = zeros(size(Ey(:,i)));
        Hz(:,i) = (-By*Ex(:,i) + Bx*Ey(:,i))/(-1i*w*mu0);
        Hy(:,i) = (i*w*eps0*Epsx*Ex(:,i) - Cy*Hz(:,i))/(1i*beta(i));
        Hx(:,i) = (i*w*eps0*Epsy*Ey(:,i) + Cx*Hz(:,i))/(-1i*beta(i));
        Ez(:,i) = (-Dy*Hx(:,i) + Dx*Hy(:,i))/(-1i*w*mu0);   
    end
